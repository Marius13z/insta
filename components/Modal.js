...
import { addDoc, setDoc, collection, serverTimestamp, updateDoc, doc } from 'firebase/firestore'
...
function Modal() {
  ...
  const uploadPost =  async () => {
     ...
     // 1. Create a post and add to firestore posts 'collection'
      ...
      // Allows us to add a document to a collection
    const docRef = await addDoc(collection(db, 'posts'), ({
     ...
     // 2. Get the ID for the newly created posts
     console.log("New doc added with ID", docRef.id)

     // 3. Upload image to firebase storage with post ID
     const imageRef = ref(storage, `posts/${docRef.id}/image`)
     
     // 4. Get download URL from firebase storage and update the original post with image
  ...
  const addImageToPost = (e) => {
    ...
    // Initialize a file reader
     ...
     // If the first file that the user selected read as a data url
     if(e.target.files[0]) {
         reader.readAsDataURL(e.target.files[0])
     }
     
     // Once browser is done with reading the file we get back reader event and store in selectedFile state
        ...
        <Dialog as='div' className="fixed z-10 inset-0 overflow-y-auto" onClose={setOpen}>
          ...
          sm:min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0 ">
             ...
             >
              ...
              pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-4 sm:align-middle
              sm:max-w-sm sm:w-full sm:p-6">
                ...
                {selectedFile ? (
                   <img className="w-full object-contain cursor-pointer" src={selectedFile} onClick={() => setSelectedFile(null)}/>
                    ...
                    className="mx-auto mt-5 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 cursor-pointer"
                    >
                    <CameraIcon 
